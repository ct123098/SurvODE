...
class CVAEWithJointCox2(CVAEBaseModel):
	def __init__(self, input_dim: int, latent_dim: int, hidden_dims=None, survival_hidden_dims=None,
				 device="cpu", kld_weight=1.0, reg_weight=1.0, cox_weight=1.0, num_epochs=1000, learning_rate=1e-3):
		super().__init__()
	...
	def training_step(self, train_batch, batch_idx):
		...
		y_order_sum = torch.sum((exp_phi.reshape(1, -1) * y_order_matrix), dim=1).reshape(-1, 1)
		num_observed = torch.sum(y_observed)
		cox_loss = -(torch.sum((phi - torch.log(y_order_sum)) * y_observed) / num_observed)
		# print(phi)
		# print(y_order_sum)
		# print((phi - torch.log(y_order_sum)))
		# print((phi - torch.log(y_order_sum)) * y_observed)
		# print(torch.sum((phi - torch.log(y_order_sum)) * y_observed))
		# print(y_observed)
		# cox_loss_gt = -(torch.sum((phi.reshape(-1) - torch.log(y_order_sum.reshape(-1))) * y_observed.reshape(-1)) / num_observed)
		# print((cox_loss / num_observed).item(), cox_loss_gt.item())
		# print(phi.norm(2))
		# exit(0)
		# T-distribution Loss
		tmp1 = torch.abs(y_value - t)
		tmp2 = torch.maximum(y_value - t, torch.zeros_like(y_value))
		reg_loss = torch.sum(tmp1 * y_observed + tmp2 * y_censored)
